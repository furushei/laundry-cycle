...
記録したCSVをスペクトログラム(時間-周波数-パワー)として可視化する`tools/plot_spectrogram.py`を用意している。
PC上で実行するツールであり、M5Stackへ転送する必要はない。
...
| オプション | 内容 |
| --- | --- |
| `--axis` | 解析対象。`magnitude`(既定、3軸のノルム。設置姿勢に依存しない)、`x`、`y`、`z` |
| `--seg-len` | STFTの窓長[サンプル]。既定256(2.56秒、Δf=0.39Hz)。長くすると周波数分解能と引き換えに時間分解能が下がる |
| `--hop` | STFTのステップ[サンプル]。既定64(0.64秒) |
| `--dynamic-range` | カラースケールの幅[dB]。既定60 |
| `-o`, `--output` | 画像ファイルへ保存する(省略時は対話的に表示) |

処理内容:

- `ticks_ms`列から実時間軸を復元する(ラップと欠測に対応)。実測サンプリング周波数と欠測区間を標準出力へ表示する
- 等間隔グリッドへ再標本化したうえで、重力と傾きドリフトを1Hzハイパスで除去する
- パワースペクトル密度[dB re 1 (m/s²)²/Hz]をプロットする
- `state`列の遷移(ボタン操作=正解ラベル)を白い破線、補間で埋めた欠測区間を赤いハッチで重ねる
